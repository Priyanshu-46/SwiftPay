...
class IdempotencyServiceTest {
    ...
    void tryReserve_newKey_returnsTrue() {
        ...
        boolean result = idempotencyService.tryReserve("key-abc", UUID.randomUUID().toString());

        assertThat(result).isTrue();
        verify(valueOps).setIfAbsent(eq("idem:key-abc"), anyString(), eq(Duration.ofSeconds(86400)));
    }
    ...
    void tryReserve_nullRedisResponse_returnsFalse() {
        ...
        boolean result = idempotencyService.tryReserve("key-null", "some-id");
    ...
    void getExistingPaymentId_keyExists_returnsValue() {
        ...
        Optional<String> result = idempotencyService.getExistingPaymentId("key-xyz");
    ...
    void getExistingPaymentId_keyMissing_returnsEmpty() {
        ...
        Optional<String> result = idempotencyService.getExistingPaymentId("missing-key");
